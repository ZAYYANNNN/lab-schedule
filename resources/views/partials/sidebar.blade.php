...
<aside class="w-72 h-screen bg-slate-900 flex flex-col fixed left-0 top-0 z-50 shadow-2xl overflow-hidden">
    ...
    <div class="px-8 py-10 relative">
        <div class="flex items-center space-x-4">
            <div
                class="w-12 h-12 bg-blue-600 shadow-lg shadow-blue-500/30 text-white rounded-2xl flex items-center justify-center animate-pulse-slow">
    ...
    <nav class="flex-1 px-4 space-y-2 overflow-y-auto scrollbar-none py-4">
        ...
        <a href="{{ route('schedules.index') }}"
            ...
            <span
                class="material-symbols-outlined text-[22px] transition-transform group-hover:scale-110">calendar_month</span>
        ...
        <a href="{{ route('labs.index') }}"
            ...
            <span
                class="material-symbols-outlined text-[22px] transition-transform group-hover:scale-110">meeting_room</span>
            <span class="text-sm font-bold tracking-tight">Lab</span>
        </a>
        ...
        <a href="{{ route('assets.index') }}"
            ...
            <span
                class="material-symbols-outlined text-[22px] transition-transform group-hover:scale-110">inventory_2</span>
            <span class="text-sm font-bold tracking-tight">Aset Lab</span>
        </a>

        @if($user->hasRentalAccess())
            <a href="{{ route('lab-rentals.index') }}"
        ...
        <a href="{{ route('borrowings.index') }}"
            ...
            <span
                class="material-symbols-outlined text-[22px] transition-transform group-hover:scale-110">shopping_bag</span>
        ...
        @if($user->role === 'superadmin')
            <div class="pt-6 pb-2">
                ...
                <a href="{{ route('users.index') }}"
                    ...
                    <span
                        class="material-symbols-outlined text-[22px] transition-transform group-hover:scale-110">manage_accounts</span>
    ...
    <div class="p-4 border-t border-slate-800 bg-slate-900/50">
        <div class="bg-slate-800/50 rounded-2xl p-4 mb-4 flex items-center gap-3 border border-slate-700/50">
            <div
                class="w-10 h-10 rounded-xl bg-blue-600 flex items-center justify-center text-white shrink-0 font-black text-xs">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button
                class="group w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-slate-800 text-slate-400 font-bold text-xs uppercase tracking-widest hover:bg-rose-600 hover:text-white transition-all">
