<x-app-layout title="Peminjaman Barang">
    ...
    <div class="max-w-[1600px] mx-auto py-6 px-4 sm:px-6 lg:px-8" x-data="borrowingPage()">
                ...
                <div class="flex flex-col md:flex-row md:items-center gap-4 justify-between">
                    ...
                    @if(in_array(auth()->user()->role, ['admin', 'superadmin']))
                        <button type="button" @click="openCreateModal()"
                            class="bg-blue-600 text-white px-5 py-3.5 rounded-2xl font-black text-[11px] uppercase tracking-wider flex items-center gap-2 shadow-lg shadow-blue-200 hover:bg-blue-700 transition-all active:scale-95 flex-shrink-0">
        ...
        <div
            ...
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <h2 class="font-black text-slate-800 uppercase text-xs tracking-[0.2em]">Data Peminjaman</h2>
                        ...
                        <tr x-show="filteredBorrowings.length === 0" x-cloak>
                            <td colspan="5" class="px-8 py-16 text-center">
                                <div class="flex flex-col items-center">
                                    <span class="material-symbols-outlined text-4xl text-slate-200 mb-3">inbox</span>
        ...
        <div
            ...
            <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-rose-50/30">
                <div class="flex items-center gap-3">
                            ...
                            <tr class="hover:bg-rose-50/30 transition-all duration-300">
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <p class="font-bold text-slate-900 text-xs mb-0.5" x-text="b.nama_peminjam">
                                            </p>
                                            <p class="text-[10px] font-bold text-slate-400" x-text="b.nim"></p>
                                        </div>
                                    </div>
                                </td>
                                <td class="px-6 py-4">
                                    <span class="font-bold text-slate-700 text-xs" x-text="b.asset?.nama"></span>
                                </td>
                                <td class="px-6 py-4">
                                ...
                                <td class="px-6 py-4">
                                    <span class="text-xs font-black text-rose-600" x-text="getLateDuration(b)"></span>
                                </td>
                            </tr>
                        </template>
                        <tr x-show="filteredLateBorrowings.length === 0" x-cloak>
                            <td colspan="5" class="px-8 py-8 text-center">
                                <p class="text-slate-400 text-xs font-bold">Tidak ada data keterlambatan.</p>
                            </td>
                        </tr>
                    </tbody>
                </table>
    ...
    <div x-show="openModal" x-transition:enter="ease-out duration-300"
        ...
        <div @click.outside="closeModal()"
            ...
            <form :action="editMode ? `/borrowings/${form.id}` : '{{ route('borrowings.store') }}'" method="POST"
                class="px-8 py-8">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                    ...
                    <div class="space-y-1">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider ml-1">NIM</label>
                        <input type="text" name="nim" x-model="form.nim" required
                            class="w-full px-4 py-3 bg-slate-50 border-slate-100 rounded-xl text-sm font-bold focus:bg-white focus:ring-2 focus:ring-blue-500 transition-all">
                    </div>
                    <div class="space-y-1">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider ml-1">Lab</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider ml-1">Aset</label>
                    ...
                    {{-- Date & Time - Borrow --}}
                    <div class="space-y-1">
                        <label class="text-[10px] font-bold text-slate-400 uppercase tracking-wider ml-1">Tgl & Jam
                            Pinjam</label>
