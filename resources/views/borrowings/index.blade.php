                ...
                <div class="bg-white rounded-[2.5rem] shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden ring-1 ring-slate-100/50">
                    ...
                    <div class="px-8 py-7 border-b border-slate-50 bg-slate-50/50">
                        ...
                        <div class="flex flex-col gap-4">
                            ...
                            <div class="mt-2">
                                <h3 class="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mb-4 ml-1 flex items-center gap-2">
                                ...
                                <div class="space-y-2 max-h-[300px] overflow-y-auto pr-2 scrollbar-thin">
                                    <template x-for="lab in allLabs" :key="lab.id">
                                        <label class="flex items-center cursor-pointer group p-2.5 hover:bg-blue-50/50 rounded-2xl border border-transparent hover:border-blue-100 transition-all duration-300">
                                            <input type="checkbox" :value="lab.id" x-model="selectedLabIds"
                                                class="w-5 h-5 rounded-lg border-slate-200 text-blue-600 focus:ring-blue-500 shadow-sm transition-all">
                                            <span class="ml-3 text-sm font-bold text-slate-700 group-hover:text-blue-600 transition" x-text="lab.name"></span>
                                        </label>
                                    </template>
                                </div>
                                <button @click="selectedLabIds = []" x-show="selectedLabIds.length > 0"
                                    class="mt-4 w-full py-3 rounded-xl border border-slate-100 text-[10px] font-black uppercase tracking-widest text-slate-400 hover:bg-slate-50 hover:text-slate-600 transition-all flex items-center justify-center gap-2">
            ...
            function borrowingPage() {
                return {
                    ...
                    get filteredBorrowings() {
                        ...
                        if (this.selectedLabIds.length > 0) {
                            filtered = filtered.filter(b => this.selectedLabIds.includes(b.lab_id) || this.selectedLabIds.includes(String(b.lab_id)));
                        }
