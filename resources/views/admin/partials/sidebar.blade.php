    ...
    <div class="px-6 py-6 border-b">
        <div class="flex items-center space-x-3">
            ...
            <div>
                ...
                <p class="text-sm text-gray-500">Admin Prodi</p>
            </div>
        </div>
    </div>
    ...
    <nav class="flex-1 px-4 py-6 space-y-1">
        ...
        <a href="{{ route('admin.dashboard') }}"
            class="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-gray-100
                {{ request()->routeIs('admin.dashboard') ? 'bg-blue-50 text-blue-600' : 'text-gray-700' }}">
        ...
        <!-- Jadwal Lab (akses dari Daftar Lab) -->
        <a href="{{ route('admin.labs.index') }}"
            class="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-gray-100
                {{ request()->routeIs('admin.labs.*') ? 'bg-blue-50 text-blue-600' : 'text-gray-700' }}">
            <span class="material-symbols-outlined">
                calendar_add_on
            </span>
        ...
        <!-- Daftar Aset (akses dari Daftar Lab) -->
        <a href="{{ route('admin.labs.index') }}"
            class="flex items-center space-x-3 px-4 py-3 rounded-lg hover:bg-gray-100
                {{ request()->routeIs('admin.labs.*') ? 'bg-blue-50 text-blue-600' : 'text-gray-700' }}">
            <span class="material-symbols-outlined">
